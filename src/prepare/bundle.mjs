...
const QUAKE_TEXTURE_LEAF_SIZING = "raster";
  ...
  try {
    ...
    if (needsTextureLeaves) {
      await runQuakeRenderBundleStepAsync("texture-wait", () =>
        waitForQuakeRenderBundleTextures(handle)
      );
    }
...
function createQuakeRenderScene(host, textureQuality) {
  ...
  return createPolyScene(host, {
    ...
    textureImageRendering: QUAKE_TEXTURE_IMAGE_RENDERING,
    textureBackend: QUAKE_TEXTURE_BACKEND,
...
async function waitForQuakeRenderBundleTextures(handle) {
  ...
  try {
    await Promise.race([
      typeof handle.whenTexturesReady === "function"
        ? handle.whenTexturesReady()
        : Promise.resolve(),
      new Promise((_, reject) => {
        timeout = window.setTimeout(() => {
          reject(new Error(quakeTextureReadinessErrorMessage(handle.element)));
        }, QUAKE_RENDER_BUNDLE_TIMEOUT_MS);
      }),
    ]);
  } finally {
    if (timeout) window.clearTimeout(timeout);
  }
  const readiness = collectPolyTextureReadiness(handle.element);
  if (!readiness.ready) throw new Error(quakeTextureReadinessErrorMessage(handle.element, readiness));
}

function quakeTextureReadinessErrorMessage(mesh, readiness = collectPolyTextureReadiness(mesh)) {
  return `Timed out waiting for PolyCSS textures: ` +
    `${readiness.readyTextureLeafCount}/${readiness.textureLeafCount} ready, ` +
    `${readiness.pendingTextureLeafCount} pending.`;
}
...
async function createQuakeAnimatedAtlasLeafHomographyPlan(handle, framePolygons) {
  ...
  for (let frameIndex = 1; frameIndex < framePolygons.length && candidates.length; frameIndex++) {
    ...
    await waitForQuakeRenderBundleTextures(handle);
...
async function tightenQuakeAtlasLeaf(leaf, boundsByKey, useExistingBounds) {
  ...
  const backgroundImage = renderBundleAtlasLeafBackgroundImage(leaf, style());
  ...
  leaf.style.background = `${backgroundImage} ` +
...
function renderBundleAtlasLeafBackgroundImage(leaf, computedStyle = null) {
  return renderBundleBackgroundImageValue(
...
function renderBundleBackgroundImageValue(...values) {
  for (const value of values) {
    const url = cssUrlValue(value);
    if (url) return cssUrlToken(url);
  }
  return "";
}
...
function cssUrlValue(value) {
  ...
  const match = /url\(\s*(?:"([^"]+)"|'([^']+)'|([^)]*?))\s*\)/.exec(text);
  return match ? cleanCssUrlValue(match[1] ?? match[2] ?? match[3] ?? "") : "";
}

function cleanCssUrlValue(value) {
  const text = String(value ?? "").trim();
  if (!text.includes("--polycss-atlas-")) return text;
  const parts = text.split(";").map((part) => part.trim()).filter(Boolean);
  for (let index = parts.length - 1; index >= 0; index--) {
    const part = parts[index];
    if (!part.startsWith("--polycss-atlas-") && !/^[a-z-]+\s*:/.test(part)) return part;
  }
  return "";
}
...
async function serializeMeshWithAssets(mesh, options = {}) {
  ...
  if (normalizeInlineAtlasLeafBoxes) {
    ...
    runQuakeRenderBundleStep("serialize-strip-atlas-leaf-metadata", () =>
      stripRenderBundleMeshMetadata(serializableMesh, {
        preserveLeafPolyIndex: Boolean(options.extractLeafStyles),
      })
    );
...
function extractRenderBundleFrameStylesFromInheritedBase(mesh, baseLeafFrameStylesByClass) {
  ...
  for (const leaf of mesh.querySelectorAll("b,i,s,u")) {
    ...
    const rawFrameStyle = (
      ...
      rawStyle.includes("--polycss-atlas-") ||
...
function directTightenedAtlasLeafFrameStyle(leaf, bounds, baseFrameStyle, options = {}) {
  ...
  const backgroundImage = renderBundleAtlasLeafBackgroundImage(leaf);
  if (
    !position ||
    !size ||
    !matrix ||
...
function renderBundleNumberAttr(element, name) {
  const rawValue = element.getAttribute(name);
  if (rawValue === null || rawValue === "") return undefined;
  const value = Number(rawValue);
...
function compactRenderBundleInheritedLeafFrameStyle(style) {
  ...
  const extras = declarations
    .filter((part) =>
      part.name !== "transform" &&
      part.name !== "background" &&
      !part.name.startsWith("background-") &&
      !part.name.startsWith("--polycss-atlas-")
    )
    .map((part) => `${part.name}:${part.value}`)
    .join(";");
...
function renderBundleLeafStyleWithExplicitAtlasSize(leaf, style, options = {}) {
  ...
  const textureHeight = renderBundleLeafTextureBoxSize(leaf, declarations, "height");
  return [
    style.replace(/;+$/, ""),
...
function renderBundleLeafTextureBoxSize(leaf, declarations, axis) {
  const attrValue = renderBundleNumberAttr(leaf, `data-polycss-texture-leaf-${axis}`);
  if (attrValue !== undefined && attrValue > 0) return `${roundCssPx(attrValue)}px`;
  const atlasAxisSize = declarations.find((part) => part.name === `--polycss-atlas-${axis}`)?.value;
  if (cssPixelValue(atlasAxisSize) !== null) return atlasAxisSize;
  const atlasSize = declarations.find((part) => part.name === "--polycss-atlas-size")?.value;
  if (cssPixelValue(atlasSize) !== null) return atlasSize;
  return "";
}
...
function stripRenderBundleMeshMetadata(mesh, options = {}) {
  ...
  if (!options.preservePolycssStyleMetadata) {
