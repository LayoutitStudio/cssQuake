...
const MAX_TRANSIENT_NAVIGATION_ATTEMPTS = 4;
...
async function runCombatCaseWithNavigationRetry(browser, baseUrl, testCase, viewport, timeoutMs) {
  for (let attempt = 1; attempt <= MAX_TRANSIENT_NAVIGATION_ATTEMPTS; attempt++) {
    try {
      return await runCombatCase(browser, baseUrl, testCase, viewport, timeoutMs);
    } catch (error) {
      if (!isTransientNavigationError(error) || attempt === MAX_TRANSIENT_NAVIGATION_ATTEMPTS) throw error;
      console.warn(
        `Retrying ${testCase.map} ${testCase.classname} #${testCase.entity} after transient navigation ` +
          `(${attempt}/${MAX_TRANSIENT_NAVIGATION_ATTEMPTS}): ${error.message}`,
      );
    }
    await sleep(250);
  }
  throw new Error(`Unreachable combat retry state for ${testCase.map} ${testCase.classname} #${testCase.entity}.`);
}
...
async function main() {
  ...
  try {
    for (const testCase of COMBAT_CASES) {
      const result = await runCombatCaseWithNavigationRetry(browser, server.url, testCase, viewport, timeoutMs);
