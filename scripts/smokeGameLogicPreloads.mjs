...
for (const mapName of mapNames) {
  ...
  checks.push([
    sameArray(derived.modelPaths, manifestModelPaths),
    `${mapName} gameLogic-derived preload model paths should match manifest model paths.\n` +
      `Derived: ${derived.modelPaths.join(", ")}\nManifest: ${manifestModelPaths.join(", ")}`,
  ]);
