...
function mapUrl(baseUrl, mapName = REPRESENTATIVE_MONSTERS[0].map) {
...
async function waitForInitialIdle(page, timeoutMs) {
  const state = await waitForDebugState(page, timeoutMs);
  if (!state.ready) throw new Error(`Initial DOM monster state timed out: ${JSON.stringify(state.last)}`);
}

async function loadMap(page, baseUrl, mapName, timeoutMs) {
  await page.goto(mapUrl(baseUrl, mapName), { waitUntil: "domcontentloaded", timeout: timeoutMs });
  const state = await waitForDebugState(page, timeoutMs, mapName);
  if (!state.ready) throw new Error(`Could not load ${mapName}: ${JSON.stringify(state.last)}`);
}

async function waitForDebugState(page, timeoutMs, expectedMap = "") {
  const started = Date.now();
  let last = null;
  while (Date.now() - started < timeoutMs) {
    try {
      last = await page.evaluate((targetMap) => {
        ...
        return {
          ...
          meshCount,
          ready: Boolean(stats && !stats.loading && meshCount > 0 && (!targetMap || stats.mapName === targetMap)),
  ...
  try {
    ...
    for (const monster of REPRESENTATIVE_MONSTERS) {
      if (monster.map !== currentMap) {
        await loadMap(page, server.url, monster.map, timeoutMs);
