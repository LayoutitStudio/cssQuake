...
const runtimeFireballsPath = path.join(projectRoot, "src/runtime/fireballs.ts");
...
const runtimeSolidGatesPath = path.join(projectRoot, "src/runtime/solidGates.ts");
...
const {
  QUAKE_FIREBALL_DAMAGE,
  QUAKE_FIREBALL_DEFAULT_SPEED,
  QUAKE_FIREBALL_DRIFT_SPEED,
  QUAKE_FIREBALL_LIFETIME_MS,
  QUAKE_FIREBALL_RADIUS,
  QUAKE_FIREBALL_SPEED_JITTER,
  quakeCreateFireballHazard,
  quakeFireballEmitterFromEntity,
  quakeFireballSpeed,
  quakeMovePointHazards,
  quakeSpawnDueFireballs,
} = await importBundledModule(runtimeFireballsPath);
...
const { quakeSolidGateActivation } = await importBundledModule(runtimeSolidGatesPath);
...
const allPreparedScenes = await readPreparedScenes(path.dirname(scenePath));
...
const preparedFuncWallFacts = allPreparedScenes.flatMap(({ map, scene: preparedScene }) =>
  sceneGameLogicEntities(preparedScene)
    .filter((entity) => entity.classname === "func_wall")
    .map((entity) => ({
      map,
      entity,
      collisionBrush: preparedScene.collision?.runtime?.brushes?.find((brush) =>
        brush.classname === "func_wall" &&
        brush.entityIndex === entity.entityIndex &&
        brush.modelIndex === entity.modelIndex
      ),
    }))
);
const targetedPreparedFuncWalls = preparedFuncWallFacts.filter(({ entity }) =>
  entity.targetname ||
  entity.target ||
...
const rebuiltE1M6Monsterjump367 = rebuiltE1M6Entities.find((entity) =>
  entity.entityIndex === 367 && entity.classname === "trigger_monsterjump"
);
...
const sceneE1M6Fireball149 = e1m6Scene.entities?.find((entity) =>
  entity.index === 149 && entity.classname === "misc_fireball"
);
...
const sceneStartFireball54 = startScene.entities?.find((entity) =>
  entity.index === 54 && entity.classname === "misc_fireball"
);
...
const sceneStartBossGate310 = startScene.entities?.find((entity) =>
  entity.index === 310 && entity.classname === "func_bossgate"
);
const sceneStartEpisodeGate344 = startScene.entities?.find((entity) =>
  entity.index === 344 && entity.classname === "func_episodegate"
);
const startBossGateActivation = sceneStartBossGate310 ? quakeSolidGateActivation(sceneStartBossGate310) : null;
const startEpisodeGateActivation = sceneStartEpisodeGate344 ? quakeSolidGateActivation(sceneStartEpisodeGate344) : null;
const startFireballEmitter54 = sceneStartFireball54
  ? quakeFireballEmitterFromEntity(sceneStartFireball54, pointToPolyForScene(startScene), 1000, () => 0.25)
  : null;
const e1m6FireballEmitter149 = sceneE1M6Fireball149
  ? quakeFireballEmitterFromEntity(sceneE1M6Fireball149, pointToPolyForScene(e1m6Scene), 1000, () => 0.25)
  : null;
const e1m6FireballHazard149 = e1m6FireballEmitter149
  ? quakeCreateFireballHazard(e1m6FireballEmitter149, 3000, randomSequence([0.25, 0.75, 0.5]))
  : null;
const dueFireballEmitter149 = e1m6FireballEmitter149 ? { ...e1m6FireballEmitter149, nextSpawnAt: 2500 } : null;
const dueFireballHazards = [];
const dueFireballCount = dueFireballEmitter149
  ? quakeSpawnDueFireballs(
    [dueFireballEmitter149],
    dueFireballHazards,
    3000,
    () => false,
    randomSequence([0.25, 0.75, 0.5, 0.2]),
  )
  : 0;
const disabledFireballEmitter149 = e1m6FireballEmitter149 ? { ...e1m6FireballEmitter149, nextSpawnAt: 2500 } : null;
const disabledFireballHazards = [];
const disabledFireballCount = disabledFireballEmitter149
  ? quakeSpawnDueFireballs(
    [disabledFireballEmitter149],
    disabledFireballHazards,
    3000,
    () => true,
    randomSequence([0.25, 0.75, 0.5, 0.2]),
  )
  : 0;
const movedFireballHazards = e1m6FireballHazard149
  ? quakeMovePointHazards(
    [
      structuredClone(e1m6FireballHazard149),
      { ...structuredClone(e1m6FireballHazard149), entityIndex: 999 },
      { ...structuredClone(e1m6FireballHazard149), entityIndex: 1000, expiresAt: 3000 },
    ],
    0.5,
    3500,
    800 * QUAKE_COLLISION_UNIT_SCALE,
    (entityIndex) => entityIndex === 999,
  )
  : [];
...
const sourceTriggerMonsterjump = rebuiltE1M6Logic?.programFacts?.entities?.trigger_monsterjump ??
  e1m6Scene.gameLogic?.programFacts?.entities?.trigger_monsterjump;
...
const checks = [
  ...
  [
    JSON.stringify(rebuiltArmorPickup20?.resolvedPickup?.behavior?.armor) === JSON.stringify({
      ...
      armorValue: 150,
      replacementScore: 90,
      replacesWhenCurrentScoreBelow: 90,
      itemFlag: 16384,
      ...
      JSON.stringify(startArmorInv271?.resolvedPickup?.behavior?.armor) === JSON.stringify({
        armorType: 0.8,
        armorValue: 200,
        replacementScore: 160,
        replacesWhenCurrentScoreBelow: 160,
        itemFlag: 32768,
  ...
  [
    runtimeRocketWeapon201.factModelPath === "progs/g_rock2.mdl" &&
      ...
      JSON.stringify(ammoOnlyInventoryEffect(runtimeRocketWeapon201.factEffect)) === JSON.stringify({ rockets: 5 }) &&
      JSON.stringify(ammoOnlyInventoryEffect(runtimeRocketWeapon201.poisonedFactEffect)) === JSON.stringify({ rockets: 5 }) &&
      runtimeRocketWeapon201.factEffect?.weapon?.id === "rocketlauncher" &&
      runtimeRocketWeapon201.factEffect?.weapon?.itemFlagExpression === "IT_ROCKET_LAUNCHER" &&
      runtimeRocketWeapon201.poisonedFactEffect?.weapon?.id === "rocketlauncher" &&
      runtimeRocketWeapon201.poisonedFactEffect?.weapon?.itemFlagExpression === "IT_ROCKET_LAUNCHER",
  ...
  [
    JSON.stringify(runtimeRocketWeaponGrantEffect) === JSON.stringify({ rockets: 5 }) &&
      JSON.stringify(ammoOnlyInventoryEffect(runtimeRocketWeapon201.factEffect)) === JSON.stringify(runtimeRocketWeaponGrantEffect) &&
      JSON.stringify(ammoOnlyInventoryEffect(runtimeRocketWeapon201.poisonedFactEffect)) === JSON.stringify(runtimeRocketWeaponGrantEffect),
    ...
    syntheticWeaponPickupAudits.length === WEAPON_PICKUP_CLASSNAMES.length &&
      syntheticWeaponPickupAudits.every((item) =>
        item.factEntity?.resolvedPickup?.behavior?.weapon &&
          ...
          JSON.stringify(ammoOnlyInventoryEffect(item.audit.factEffect)) === JSON.stringify(item.grantEffect) &&
          JSON.stringify(ammoOnlyInventoryEffect(item.audit.fallbackEffect)) === JSON.stringify(item.grantEffect) &&
          JSON.stringify(ammoOnlyInventoryEffect(item.audit.poisonedFactEffect)) === JSON.stringify(item.grantEffect) &&
          JSON.stringify(ammoOnlyInventoryEffect(item.audit.poisonedFallbackEffect)) === JSON.stringify(item.grantEffect) &&
          item.audit.factEffect?.weapon?.itemFlag === item.factEntity.resolvedPickup.behavior.weapon.itemFlag &&
          item.audit.factEffect?.weapon?.itemFlagExpression === item.factEntity.resolvedPickup.behavior.weapon.itemFlagExpression
      ),
  ...
  [
    preparedFuncWallFacts.length === 43 &&
      preparedFuncWallFacts.every(({ entity }) =>
        entity.category === "brush" &&
        entity.runtimeStatus === "active" &&
        typeof entity.modelIndex === "number" &&
        entity.brushModel?.kind === "solid"
      ),
    "prepared func_wall facts should cover all shareware static solid brush entities",
  ],
  [
    preparedFuncWallFacts.every(({ collisionBrush }) => collisionBrush?.kind === "solid"),
    "prepared func_wall collision facts should expose solid static brush hulls",
  ],
  [
    JSON.stringify(targetedPreparedFuncWalls.map(({ map, entity }) => ({
      map,
      entityIndex: entity.entityIndex,
      targetname: entity.targetname ?? entity.properties?.targetname,
    }))) === JSON.stringify([{ map: "start", entityIndex: 379, targetname: "t10" }]),
    "prepared func_wall audit should document the only targeted wall without claiming dynamic func_wall_use support",
  ],
  [
  ...
  [
    e1m1SecretDoor97?.resolvedMover?.kind === "func_door_secret" &&
      e1m1SecretDoor97.resolvedMover.speed === 50 &&
      e1m1SecretDoor97.resolvedMover.wait === 5 &&
      e1m1SecretDoor97.resolvedMover.dmg === 2 &&
      e1m1SecretDoor97.resolvedMover.sounds === 3 &&
      e1m1SecretDoor97.resolvedMover.soundSet?.activate === "doors/basesec2.wav" &&
      e1m1SecretDoor97.resolvedMover.soundSet?.move === "doors/basesec1.wav" &&
      e1m1SecretDoor97.resolvedMover.soundSet?.stop === "doors/basesec2.wav" &&
      e1m1SecretDoor97.resolvedMover.openOnce === true &&
      e1m1SecretDoor97.resolvedMover.firstMoveLeft === true &&
      e1m1SecretDoor97.resolvedMover.firstMoveDown === false &&
      e1m1SecretDoor97.resolvedMover.noShoot === false &&
      e1m1SecretDoor97.resolvedMover.alwaysShoot === false &&
      e1m1SecretDoor97.resolvedMover.shootableAtSpawn === false &&
      e1m1SecretDoor97.resolvedMover.disablesShootOnUse === true &&
      e1m1SecretDoor97.resolvedMover.motion.firstMove === "left" &&
      e1m1SecretDoor97.resolvedMover.motion.widthExpression === "fabs(v_right * self.size)" &&
      e1m1SecretDoor97.resolvedMover.motion.lengthExpression === "fabs(v_forward * self.size)" &&
      e1m1SecretDoor97.resolvedMover.motion.sourceFunctions.includes("fd_secret_move6") &&
      e1m1SecretDoor132?.resolvedMover?.kind === "func_door_secret" &&
      e1m1SecretDoor132.resolvedMover.openOnce === false &&
      e1m1SecretDoor132.resolvedMover.firstMoveLeft === true &&
      e1m1SecretDoor132.resolvedMover.shootableAtSpawn === true &&
      rebuiltE1M2SecretDoor460?.resolvedMover?.kind === "func_door_secret" &&
      rebuiltE1M2SecretDoor460.resolvedMover.openOnce === true &&
      rebuiltE1M2SecretDoor460.resolvedMover.motion.firstMove === "right",
    "func_door_secret should resolve source-backed mover facts without changing runtime behavior",
  ],
  [
    e1m1SecretDoor97?.resolvedMover?.kind === "func_door_secret" &&
      e1m1SecretDoor97.resolvedMover.callbacks.blocked === "secret_blocked" &&
      e1m1SecretDoor97.resolvedMover.callbacks.touch === "secret_touch" &&
      e1m1SecretDoor97.resolvedMover.callbacks.use === "fd_secret_use" &&
      e1m1SecretDoor97.resolvedMover.callbacks.th_pain === "fd_secret_use" &&
      e1m1SecretDoor97.resolvedMover.blocker.sourceFunction === "secret_blocked" &&
      e1m1SecretDoor97.resolvedMover.blocker.damageField === "dmg" &&
      e1m1SecretDoor97.resolvedMover.blocker.damage === 2 &&
      e1m1SecretDoor97.resolvedMover.blocker.damageExpression === "self.dmg" &&
      e1m1SecretDoor97.resolvedMover.blocker.reverses === false &&
      e1m1SecretDoor97.resolvedMover.blocker.throttleSeconds === 0.5 &&
      e1m1SecretDoor97.resolvedMover.blocker.throttleField === "attack_finished" &&
      e1m1SecretDoor97.resolvedMover.blocker.throttleExpression === "time + 0.5",
    "func_door_secret blocker facts should expose source damage, throttle, and no-reverse shape",
  ],
  [
    runtimeE1M1SecretDoor97?.kind === "secret-door" &&
      ...
      runtimeE1M1SecretDoor97.prebakedSecretDoor?.kind === "func_door_secret" &&
      ...
      runtimeE1M1SecretDoor132.prebakedSecretDoor?.kind === "func_door_secret",
    "runtime mover setup should keep func_door_secret as secret-door states with secret-door facts",
  ],
  [
  ...
  [
    startFireballEmitter54?.entityIndex === 54 &&
      sameArray(startFireballEmitter54.origin, scaledPointForScene(startScene, sceneStartFireball54?.origin)) &&
      startFireballEmitter54.speed === 600 * QUAKE_COLLISION_UNIT_SCALE &&
      startFireballEmitter54.nextSpawnAt === 2250 &&
      e1m6FireballEmitter149?.entityIndex === 149 &&
      sameArray(e1m6FireballEmitter149.origin, scaledPointForScene(e1m6Scene, sceneE1M6Fireball149?.origin)) &&
      e1m6FireballEmitter149.speed === 1000 * QUAKE_COLLISION_UNIT_SCALE &&
      e1m6FireballEmitter149.nextSpawnAt === 2250 &&
      quakeFireballSpeed(0) === QUAKE_FIREBALL_DEFAULT_SPEED * QUAKE_COLLISION_UNIT_SCALE,
    "runtime fireball helper should derive emitter origin, speed, and initial spawn time from misc_fireball entities",
  ],
  [
    e1m6FireballHazard149?.entityIndex === 149 &&
      e1m6FireballHazard149.damage === QUAKE_FIREBALL_DAMAGE &&
      e1m6FireballHazard149.kind === "fireball" &&
      e1m6FireballHazard149.radiusSq === QUAKE_FIREBALL_RADIUS * QUAKE_FIREBALL_RADIUS &&
      e1m6FireballHazard149.expiresAt === 3000 + QUAKE_FIREBALL_LIFETIME_MS &&
      sameArray(e1m6FireballHazard149.origin, e1m6FireballEmitter149?.origin) &&
      sameArray(e1m6FireballHazard149.velocity, [
        -0.5 * QUAKE_FIREBALL_DRIFT_SPEED,
        0.5 * QUAKE_FIREBALL_DRIFT_SPEED,
        e1m6FireballEmitter149.speed + 0.5 * QUAKE_FIREBALL_SPEED_JITTER,
      ]),
    "runtime fireball helper should create deterministic fireball hazard shape",
  ],
  [
    dueFireballCount === 1 &&
      dueFireballHazards.length === 1 &&
      dueFireballEmitter149?.nextSpawnAt === 7000 &&
      disabledFireballCount === 0 &&
      disabledFireballHazards.length === 0 &&
      disabledFireballEmitter149?.nextSpawnAt === 2500,
    "runtime fireball helper should spawn only due enabled emitters and preserve disabled emitters",
  ],
  [
    movedFireballHazards.length === 1 &&
      movedFireballHazards[0].entityIndex === 149 &&
      sameArray(movedFireballHazards[0].velocity, [
        -0.5 * QUAKE_FIREBALL_DRIFT_SPEED,
        0.5 * QUAKE_FIREBALL_DRIFT_SPEED,
        e1m6FireballEmitter149.speed + 0.5 * QUAKE_FIREBALL_SPEED_JITTER - 400 * QUAKE_COLLISION_UNIT_SCALE,
      ]) &&
      sameArray(movedFireballHazards[0].origin, [
        e1m6FireballHazard149.origin[0] - 0.25 * QUAKE_FIREBALL_DRIFT_SPEED,
        e1m6FireballHazard149.origin[1] + 0.25 * QUAKE_FIREBALL_DRIFT_SPEED,
        e1m6FireballHazard149.origin[2] +
          (e1m6FireballEmitter149.speed + 0.5 * QUAKE_FIREBALL_SPEED_JITTER - 400 * QUAKE_COLLISION_UNIT_SCALE) * 0.5,
      ]),
    "runtime fireball helper should move active hazards and drop disabled or expired hazards",
  ],
  [
  ...
  [
    startEpisodeGateActivation?.kind === "episode" &&
      startEpisodeGateActivation.message === "The Doomed Dimension\nhas been completed!" &&
      startBossGateActivation?.kind === "boss" &&
      startBossGateActivation.message === "YOU MUST COMPLETE THE EPISODES FIRST",
    "runtime solid gate helper should resolve episode and boss gate centerprint messages",
  ],
  [
  ...
  [
    sourceTriggerMonsterjump?.defaultAssignments?.some((assignment) => assignment.field === "speed" && assignment.value === 200) &&
      sourceTriggerMonsterjump.defaultAssignments?.some((assignment) => assignment.field === "height" && assignment.value === 200) &&
      sourceTriggerMonsterjump.fieldAssignments?.some((assignment) =>
        assignment.field === "angles" &&
        JSON.stringify(assignment.value) === JSON.stringify([0, 360, 0])
      ) &&
      sourceTriggerMonsterjump.callbackFacts?.trigger_monsterjump_touch?.assignments?.some((assignment) =>
        assignment.field === "other.velocity_x" &&
        assignment.expression === "self.movedir_x * self.speed"
      ) &&
      sourceTriggerMonsterjump.callbackFacts?.trigger_monsterjump_touch?.assignments?.some((assignment) =>
        assignment.field === "other.velocity_y" &&
        assignment.expression === "self.movedir_y * self.speed"
      ) &&
      sourceTriggerMonsterjump.callbackFacts?.trigger_monsterjump_touch?.assignments?.some((assignment) =>
        assignment.field === "other.velocity_z" &&
        assignment.expression === "self.height"
      ) &&
      sourceTriggerMonsterjump.callbackFacts?.trigger_monsterjump_touch?.assignments?.some((assignment) =>
        assignment.field === "other.flags" &&
        assignment.expression === "other.flags - FL_ONGROUND"
      ),
    "source-backed trigger_monsterjump facts should include defaults, angle fallback, and touch assignment expressions",
  ],
  [
  ...
  [
    e1m6Monsterjump367?.resolvedTrigger?.kind === "trigger_monsterjump" &&
      ...
      e1m6Monsterjump367.resolvedTrigger.monsterJump?.source.touchFunction === "trigger_monsterjump_touch" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.source.defaultAnglesExpression === "'0 360 0'" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.actorFilter.requiredFlags.includes("FL_MONSTER") &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.actorFilter.excludedFlags.includes("FL_FLY") &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.actorFilter.excludedFlags.includes("FL_SWIM") &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.horizontal.speed === 200 &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.horizontal.xExpression === "self.movedir_x * self.speed" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.horizontal.yExpression === "self.movedir_y * self.speed" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.horizontal.appliesBeforeGroundCheck === true &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.vertical.height === 200 &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.vertical.zExpression === "self.height" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.vertical.requiresGroundFlag === "FL_ONGROUND" &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.vertical.clearGroundExpression === "other.flags - FL_ONGROUND" &&
      closeVector(e1m6Monsterjump367.resolvedTrigger.monsterJump.defaultAngles, { x: 0, y: 360, z: 0 }) &&
      JSON.stringify(e1m6Monsterjump367.resolvedTrigger.monsterJump.modeMask) === JSON.stringify([
        "singleplayer:easy",
        "singleplayer:normal",
        "singleplayer:hard",
      ]) &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.triggerBrush?.modelIndex === 83 &&
      e1m6Monsterjump367.resolvedTrigger.monsterJump.triggerBrush.kind === "trigger" &&
      JSON.stringify(e1m6Monsterjump367.resolvedTrigger.monsterJump.triggerBrush.mins) ===
        JSON.stringify({ x: -511, y: 1473, z: 65 }) &&
      JSON.stringify(e1m6Monsterjump367.resolvedTrigger.monsterJump.triggerBrush.maxs) ===
        JSON.stringify({ x: -193, y: 1479, z: 127 }) &&
      rebuiltE1M6Monsterjump367?.resolvedTrigger?.monsterJump?.horizontal.speed ===
        e1m6Monsterjump367.resolvedTrigger.monsterJump.horizontal.speed,
    "E1M6 trigger_monsterjump should resolve QuakeC speed/height defaults, touch callback, push rule, and trigger bounds",
  ],
  [
...
async function readPreparedScenes(dirPath) {
  const files = (await readdir(dirPath))
    .filter((file) => file.endsWith(".json"))
    .sort();
  const scenes = await Promise.all(files.map(async (file) => {
    const preparedScene = await readScene(path.join(dirPath, file));
    return {
      map: path.basename(file, ".json"),
      scene: preparedScene,
    };
  }));
  return scenes.filter(({ scene: preparedScene }) => Array.isArray(preparedScene?.gameLogic?.entities));
}
...
function scaledPointForScene(inputScene, point) {
  if (!point) return [];
  const pivot = inputScene.collision?.pivot ?? { x: 0, y: 0, z: 0 };
  return [
...
function pointToPolyForScene(inputScene) {
  return (point) => scaledPointForScene(inputScene, point);
}
